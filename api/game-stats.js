export default async function handler(req, res) {
  ...
  try {
    ...
    // get tier image URL + MMR history (RR changes) in parallel
    var tierImageUrl = "";
    var mmrHistory = {};
    var rrGraph = [];
    try {
      ...
      if (mmrRes.ok) {
        ...
        // RR history from by_season
        var history = (mmrJson.data && mmrJson.data.by_season) ? [] : [];
      }
    } catch(_) {}

    // fetch MMR history for RR changes per match
    try {
      var mmrHistRes = await fetch(
        "https://api.henrikdev.xyz/valorant/v1/mmr-history/" + region + "/" + encodeURIComponent(name) + "/" + encodeURIComponent(tag),
        { headers: { "Authorization": henrikApiKey } }
      );
      if (mmrHistRes.ok) {
        var mmrHistJson = await mmrHistRes.json();
        var cutoff = Date.now() - 14 * 24 * 60 * 60 * 1000;
        (mmrHistJson.data || []).forEach(function(h) {
          if (h.match_id) {
            mmrHistory[h.match_id] = {
              change: h.mmr_change_to_last_game || 0,
              tier: h.currenttierpatched || ""
            };
          }
          // RR history within 14 days
          if (h.date_raw && h.date_raw * 1000 >= cutoff) {
            rrGraph.push({
              date: h.date_raw,
              rr: h.ranking_in_tier || 0,
              tier: h.currenttierpatched || "",
              change: h.mmr_change_to_last_game || 0
            });
          }
        });
        rrGraph.sort(function(a, b) { return a.date - b.date; });
      }
    } catch(_) {}
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      var matchId = (match.metadata && match.metadata.matchid) ? match.metadata.matchid : "";
      var rrChange = mmrHistory[matchId] !== undefined ? mmrHistory[matchId].change : null;
