  ...
  try {
    ...
    const recentMatches = matches.map(match => {
      ...
      return {
        ...
        mapImageUrl: match.metadata?.map_id
          ? `https://media.valorant-api.com/maps/${match.metadata.map_id}/splash.png`
          : null,
...
function calculateStats(matches, playerName, playerTag) {
  ...
  matches.forEach(match => {
    ...
    if (!mapStats[map]) mapStats[map] = { games: 0, wins: 0, imageUrl: null };
    mapStats[map].games++;
    if (won) mapStats[map].wins++;
    // Henrik API에서 맵 이미지 URL 추출
    if (!mapStats[map].imageUrl && match.metadata?.map_id) {
      mapStats[map].imageUrl = `https://media.valorant-api.com/maps/${match.metadata.map_id}/splash.png`;
    }
  });
