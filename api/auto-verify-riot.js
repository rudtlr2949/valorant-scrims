...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Henrik API 시도
    let henrikRes = await fetch(
      `https://api.henrikdev.xyz/valorant/v3/by-puuid/mmr/ap/${puuid}`
    );
    
    // Henrik 실패 시 이름/태그로 직접 조회 (v2 fallback)
    if (!henrikRes.ok && henrikRes.status === 401) {
      console.log('Henrik v3 실패, v2 fallback 시도...');
      henrikRes = await fetch(
    ...
    if (!henrikRes.ok) {
      ...
      console.error('Henrik API 에러:', henrikRes.status, errorText);
    ...
    if (!henrikData.data || !henrikData.data.current_data) {
      return res.status(200).json({ 
        success: true, 
        tier: 'Unranked', 
        rr: 0,
    ...
    const currentData = henrikData.data.current_data;
