...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!trackerRes.ok) {
      const errorText = await trackerRes.text();
      console.error('Tracker.gg API 에러:', trackerRes.status, errorText);
      ...
      throw new Error(`랭크 정보 조회 실패 (${trackerRes.status})`);
    }
    ...
    const tierValue = compSegment.stats.tier.metadata?.tierName || 'Unranked';
    const rr = compSegment.stats.rankRating?.value || 0;
    
    // Tracker.gg 티어 형식: "Diamond 1" → "Diamond 1"로 변환
    const tierName = tierValue.replace(/\s+/g, ' '); // 공백 정규화
